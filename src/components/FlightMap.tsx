...
const NETWORK_COLOR: Record<Network, string> = {
  vatsim: "#4da3ff", // VATSIM (bleu ravivé)
  ivao: "#ff5c8a", // IVAO (magenta ravivé)
};
...
export default function FlightMap({ network }: { network: Network }) {
  ...
  return (
    <div className="relative w-full h-full bg-chart-paper">
      {loading && (
        <div
      ...
      {snapshot && !error && (
        <div
          className="absolute top-4 left-4 z-[1000] rounded-2xl border border-white/10 px-4 py-2.5 font-mono text-xs font-bold tracking-[.14em] flex gap-5"
          style={{
            background: "linear-gradient(180deg,rgba(30,42,66,.82),rgba(18,27,45,.82))",
            backdropFilter: "blur(10px)",
            boxShadow: "0 12px 30px -10px rgba(0,0,0,.75), inset 0 1px 0 rgba(255,255,255,.12)",
          }}
        >
          <span style={{ color: accent }}>
            ✈ {String(snapshot.pilots.length).padStart(4, "0")} VOLS
          </span>
          <span className="text-chart-ink-dim inline-flex items-center gap-1.5">
            <svg viewBox="0 0 485 485" width="13" height="13" fill="currentColor" style={{ flexShrink: 0 }}>
              <path d="M450.463,211.887h-94.811l23.411-115.613H257.5V0h-30v96.274H105.938l23.411,115.613H34.537L64.022,357.5h121.925V485h30V357.5h53.105V485h30V357.5h121.925L450.463,211.887z M257.5,241.887h63.215l-8.668,85.613H257.5V241.887z M227.5,327.5h-54.547l-8.668-85.613H227.5V327.5z M142.621,126.274h199.758l-17.335,85.613H159.956L142.621,126.274z M71.221,241.887h62.911l8.668,85.613H88.557L71.221,241.887z M396.443,327.5h-54.242l8.668-85.613h62.911L396.443,327.5z" />
            </svg>
